...
public:
    ...
    void draw(Core* core, VertexShaderCBAnimatedModel *vsCB) {
        ...
        shaderManager->updateConstantVS("AnimatedVertexShader.hlsl", "W",  &vsCB->W);
        shaderManager->updateConstantVS("AnimatedVertexShader.hlsl", "VP", &vsCB->VP);
        shaderManager->updateConstantVS("AnimatedVertexShader.hlsl", "bones", vsCB->bones);
